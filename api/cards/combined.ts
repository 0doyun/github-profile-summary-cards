import {getCombinedSVGWithThemeName} from '../../src/cards/combined-card';
...
export default async (req: VercelRequest, res: VercelResponse) => {
    ...
    try {
        ...
        while (true) {
            try {
                const cardSVG = await getCombinedSVGWithThemeName(username, theme, Number(utcOffset), token);
                await sendAnalytics('combined-card', {username, theme, utcOffset}, req.headers);
